...
module.exports = {
	...
	var dataObj = {};
		...
		request(EMOTION_URL + queryString, function (error, response, body) {
			if(error) {
        ...
        var emotionObj = {
	      ...
	      request(SENTIMENT_URL + queryString, function (error, response, body) {
				  if(error) {
					  console.log("error", error)
				  } else {  
						var sentimentObj = {
	            ...
	            _id: rawData._id
			      ...
			      dataObj[emotionObj] = emotionObj;
			      dataObj[sentimentObj] = sentimentObj;
